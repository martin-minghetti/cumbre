...
import { z } from 'zod';
import { removeLine, signCart, verifyCart, type Cart } from '@/lib/cart';
import { rateLimit, clientIp } from '@/lib/rate-limit';

const bodySchema = z.object({ packId: z.number().int().positive() });
...
export async function POST(req: NextRequest) {
  ...
  const next = removeLine(cart, parsed.data.packId);
